...
Config {
    ...
    lowerOnStart = True,
    commands = [
        Run Weather "KPAO" ["-t","<tempF>F <skyCondition>","-L","64","-H","88","--normal","#CEFFAC","--high","#FFB6B0","--low","#96CBFE"] 36000,
    ...
    sepChar = "%",
    alignSep = "}{",
    template = "%StdinReader% }{ %multicpu%   %memory%   %swap%   %eth0%   %stocks%  <fc=#FFFFCC>%date%</fc>   %KPAO%"
}
